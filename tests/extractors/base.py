...
class TestExtractionBase(BaseMockTests):
    ...
    def loadData(self):
        ...
        """
        test, suite, module, cls, func = self.id().split('.')
        path = os.path.join(os.path.dirname(CURRENT_PATH), "data", suite, module, "%s.json" % func)
        path = os.path.abspath(path)
        content = FileHelper.loadResourceFile(path)
    ...
    def load_test_file(self, suffix):
        test, suite, module, cls, func = self.id().split('.')
        path = os.path.join(os.path.dirname(CURRENT_PATH), "data", suite, module, "%s%s" % (func, suffix))
        path = os.path.abspath(path)
        try:
